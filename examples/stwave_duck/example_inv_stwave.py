...
m = np.prod(N)
dx = np.array([5.0, 5.0])
xmin = np.array([0.0 + dx[0] / 2.0, 0.0 + dx[1] / 2.0])
xmax = np.array([110.0 * 5.0 - dx[0] / 2.0, 83.0 * 5.0 - dx[1] / 2.0])

# covairance kernel and scale parameters
# following Hojat's paper
prior_std = 1.5
prior_cov_scale = np.array([18.0 * 5.0, 18.0 * 5.0])
...
# for plotting
x = np.linspace(0.0 + dx[0] / 2.0, 110 * 5 - dx[0] / 2.0, N[0])
y = np.linspace(0.0 + dx[1] / 2.0, 83 * 5 - dx[0] / 2.0, N[1])
...
# params['ncores'] = 36, with parallel True, it will determine maximum
# physical core unless specified
...
post_diagv[post_diagv < 0.0] = 0.0  # just in case
...
fig = plt.figure()
im = plt.imshow(
    np.flipud(np.fliplr(post_std2d)), extent=[0, 110, 0, 83], cmap=plt.get_cmap("jet")
)
...
fig.suptitle(
    "transect with prior var.: (%g)^2, n_pc : %d, lx = %f m, ly = %f m"
...
for i in range(4):
    for j in range(4):
        axes[i, j].imshow(
            prob.priorU[:, (i * 4 + j) * 2].reshape(N[1], N[0]), extent=[0, 110, 0, 83]
        )
