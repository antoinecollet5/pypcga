...
class Model:
    ...
    def run_model_single(self, logHK, Q_loc, idx=0):
        ...
        exec_name = os.path.abspath(
            os.path.join(self.homedir, self.input_dir, self.mf_exec)
        )
        mymf = flopy.modflow.Modflow(
            modelname=modelname, exe_name=exec_name, model_ws=mydir
        )
        ...
        dis = flopy.modflow.ModflowDis(
            mymf, nlay, nrow, ncol, delr=delr, delc=delc, top=ztop, botm=botm[1:]
        )
...
if __name__ == "__main__":
    ...
    for i in range(nrelzs):
        logHKrelz[:, i : i + 1] = logHK + 0.1 * np.random.randn(
            np.size(logHKrelz, 0), 1
        )
