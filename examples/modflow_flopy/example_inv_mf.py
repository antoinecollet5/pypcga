...
m = np.prod(N)
dx = np.array([10.0, 10.0, 1.0])
xmin = np.array([0.0 + dx[0] / 2.0, 0.0 + dx[1] / 2.0, 0.0 + dx[2] / 2.0])
xmax = np.array([Lx - dx[0] / 2.0, Ly - dx[1] / 2.0, Lz - dx[2] / 2.0])
...
prior_std = 1.0
prior_cov_scale = np.array([200.0, 200.0, 1.0])
...
# for plotting
x = np.linspace(0.0 + dx[0] / 2.0, Lx - dx[0] / 2.0, N[0])
y = np.linspace(0.0 + dx[1] / 2.0, Ly - dx[1] / 2.0, N[1])
...
params = {
    ...
    "post_cov": "diag",
    "precond": True,
    "LM": True,  #'LM_smin' : 1.0, 'LM_smax' : 4.0,
    "parallel": True,
    "linesearch": True,
...
im = axes[0].pcolormesh(
    XX, YY, s_true2d, vmin=minv, vmax=maxv, cmap=plt.get_cmap("jet")
)
