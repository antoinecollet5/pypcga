...
pcm = ax[0].pcolormesh(
    xx, yy, s_true.reshape(128, 128), cmap=plt.cm.get_cmap("jet"), vmin=-14, vmax=-7.5
)
...
fig = plt.figure()
plt.title(
    "pressure - obs. vs simul., RMSE = %f"
    % (np.sqrt(((obs - simulobs) ** 2).sum() / nobs))
)
