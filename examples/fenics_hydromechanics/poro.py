...
# from rbnics.backends.dolfin.wrapping.function_save import SolutionFileXDMF
# as SolutionFile
...
from ufl import FiniteElement, VectorElement, avg, div, grad, inner, jump, sym
...
class Model:
    ...
    def model_run(self, mu, idx=0):
        ...
        bcd1 = DirichletBC(
            W.sub(0).sub(0), Constant(0.0), boundaries, 1
        )  # No normal displacement for solid on left side
        bcd3 = DirichletBC(
            W.sub(0).sub(0), Constant(0.0), boundaries, 3
        )  # No normal displacement for solid on right side
        bcd4 = DirichletBC(
            W.sub(0).sub(1), Constant(0.0), boundaries, 4
        )  # No normal displacement for solid on bottom side
        ...
        d = (
            phi * rho * ct * p * q * dx
            + dt * dot(rho * k / vis * grad(p), grad(q)) * dx
            - dt * dot(avg_w(rho * k / vis * grad(p), weight_e(k, n)), jump(q, n)) * dS
            - theta
            * dt
            * dot(avg_w(rho * k / vis * grad(q), weight_e(k, n)), jump(p, n))
            * dS
            + dt
            * penalty1
            / h_avg
        ...
        f_p = (
            ...
            + dt
            * (penalty2 / h * rho / vis * dot(dot(n, k), n) * dot(p_D1 * n, q * n))
            * ds(2)
            - dt * dot(p_D2 * n, rho * k / vis * grad(q)) * ds(4)
            + dt
            * (penalty2 / h * rho / vis * dot(dot(n, k), n) * dot(p_D2 * n, q * n))
            * ds(4)
        )
