...
# forward model information: model domain, discretization,
# observation location and interval
nx = [30, 10, 10]
dx = [10.0, 10.0, 2.0]
...
xmax = np.array([300.0, 100.0, 20.0])
...
# params['ncores'] = 36, with parallel True, it will determine maximum physical core
# unless specified
...
xmin, xmax = (
    math.floor(minobs / interval_pres) * interval_pres,
    math.ceil(maxobs / interval_pres) * interval_pres,
)
...
xmin, xmax = (
    math.floor(minobs / interval_temp) * interval_temp,
    math.ceil(maxobs / interval_temp) * interval_temp,
)
