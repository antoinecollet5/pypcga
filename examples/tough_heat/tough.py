...
class Model:
    def __init__(self, params=None):
        ...
        from psutil import cpu_count  # physical cpu counts
        ...
        if params is not None:
            ...
            if "homedir" in params:
                ...
                self.inputdir = os.path.abspath(
                    os.path.join(self.homedir, "./input_files")
                )
            ...
            if not params.keys() != required_params:
                raise ValueError(
                    "You need to provide all the required tough parameters"
                )
    ...
    def run_model(self, s, idx=0):
        ...
        dat.parameter.update(
            {
                ...
                "default_incons": self.default_incons,
            }
        )  # default initial conditions
        # Pressure in Pa, 100 m water = 10.e5 Pa water, 10 is the temperature in Celsius
        dat.start = True
        ...
        dat.relative_permeability = {
            "type": 7,
            "parameters": [0.95, 0.5, 1.0, 0.25, 0.0],
        }
        dat.capillarity = {"type": 7, "parameters": [0.8, 0.32, 1.0e-4, 1.0e7, 0.999]}

        # rocktype object, Table 3.3, page 67 PyTOUGH
        r1 = rocktype(
            "dfalt",
            permeability=[0.0e-13] * 3,
            density=2600,
            porosity=0.25,
            conductivity=2.51,
            specific_heat=920,
        ...
        r2 = rocktype(
            "HOMO1",
            permeability=[9.0e-13, 3.0e-14, 3.0e-14],
            density=2600,
            porosity=0.25,
            conductivity=2.51,
            specific_heat=920,
        ...
        subprocess.call(
            ["mpirun", "-n", "6", "--bind-to", "none", "tough2-mp-eos1.debug"],
    ...
    def read_FOFT(self, cur_dir):
        ...
        FOFT_files = [
            filename for filename in os.listdir(".") if filename.startswith("FOFT")
        ]
        columns_name = [
            "Element",
            "Time",
        ...
        for filename in FOFT_files:
            df = pd.read_fwf(
                filename,
                colspecs=[(12, 16), (21, 35), (37, 52), (54, 69), (72, 86)],
                skiprows=[0, 1],
                names=columns_name,
            )
            frame.append(df)
        ...
        frame[["Time", "Gas Pressure", "Gas Saturation", "Temperature"]] = frame[
            ["Time", "Gas Pressure", "Gas Saturation", "Temperature"]
        ].apply(pd.to_numeric)
    ...
    def heat_generator(self, dat, geo, center, L, qmax, L_scale, method="Square"):
        ...
        if method == "Square":
            ...
            corlength = 0.5
            qcol = qmax * np.exp(
                -0.5 * ((dxs * dxs + dys * dys) / (corlength * corlength))
            )
            # print(qcol)
            ...
            dat.add_generator(
                t2generator(name=" in1", block=" d125", type="COM1", gx=0.6, ex=3.0e5)
            )
            dat.add_generator(
                t2generator(name=" in2", block=" e125", type="COM1", gx=0.6, ex=3.0e5)
            )
            dat.add_generator(
                t2generator(name=" in3", block=" f125", type="COM1", gx=0.6, ex=3.0e5)
            )
            dat.add_generator(
                t2generator(name=" in4", block=" g125", type="COM1", gx=0.6, ex=3.0e5)
            )

            dat.add_generator(
                t2generator(name=" ex1", block=" d145", type="COM1", gx=-0.6)
            )
            dat.add_generator(
                t2generator(name=" ex2", block=" e145", type="COM1", gx=-0.6)
            )
            dat.add_generator(
                t2generator(name=" ex3", block=" f145", type="COM1", gx=-0.6)
            )
            dat.add_generator(
                t2generator(name=" ex4", block=" g145", type="COM1", gx=-0.6)
            )
    ...
    def read_SAVE(self):
        ...
        data_SAVE = pd.read_csv(
            "SAVE", sep="\s+", header=None, skiprows=[0], error_bad_lines=False
        )
        ...
        data_save = {"Saturation": blk_1, "Pressure": blk_2, "Temperature": blk_3}
    ...
    def plot_SAVE(
        self, dat, df, nx, y=0, col="Temperature", clim=[0, 100], figname=None
    ):
        """plot_SAVE"""
...
if __name__ == "__main__":
    ...
    params = {
        ...
        "ylocs": ylocs,
        "zlocs": zlocs,
        "obs_type": ["Gas Pressure", "Temperature"],
        "t_obs_interval": 86400.0 * 5.0,
        "max_timesteps": 9000,
        "tstop": 0.32342126e08,
        "const_timestep": 6,
        "max_timestep": 86400,
        ...
        "print_interval": 9000,
        "timestep_reduction": 3.0,
        "gravity": 9.81,
        "default_incons": [100.0e4, 10],
        "multi_params": {
            "num_components": 1,
            "num_equations": 2,
            "num_phases": 2,
        ...
        "solver_params": {
            ...
            "z_precond": 1,
            "o_precond": 0,
            "relative_max_iterations": 0.8,
            "closure": 1.0e-7,
        },
        "output_times_params": {
            "num_times_specified": 2,
            "time": [0.8640e04, 0.32342126e08],
        },
    }
