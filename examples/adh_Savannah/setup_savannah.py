...
import numpy as np
import tables
from enkf_tools import Mesh_lite, buildProblemSimulations
...
class SavannahRiver(buildProblemSimulations.LogicallyRectangular_Simulation_Base):
    ...
    def __init__(
        ...
        ##index space
        x0 = [index_rect[0, 0], index_rect[1, 0], 0.0]
        xL = [index_rect[0, 1], index_rect[1, 1], 0.0]
        geometry = buildProblemSimulations.LogicallyRectangularGeometry(
            Lx=xL[0] - x0[0], Ly=xL[1] - x0[1], x0=x0, v_drop=0.0
        )
        buildProblemSimulations.LogicallyRectangular_Simulation_Base.__init__(
            self, geometry, mesh
        )
...
class SavannahRiverProblem(swp.ShallowWater2D_SimulationBasedProblem_AdH):
    ...
    def __init__(
        ...
        swp.ShallowWater2D_SimulationBasedProblem_AdH.__init__(
            self,
            mesh,
            velocity_obs_loc,
            elev_obs_loc,
        ...
        for suffix in [".3dm", ".bc", ".hot"]:
            assert os.path.isfile(sim_prefix + suffix), (
                sim_prefix + suffix + " not found"
            )
        ##make sure that true solution files exist
        assert os.path.isfile(true_soln_file_h5)
        for suffix in [".3dm", ".xmf"]:
            assert os.path.isfile(true_soln_meshbase + suffix), (
                true_soln_meshbase + suffix + " not found"
            )
    ...
    def generate_initial_ensemble(self):
        ...
        if self.initial_ensemble_flag == 0:  # readfromfile
            ...
            for i in range(self.nrens):
                Ensembles[0 : self.nn, i] = np.random.randn(self.nn) * np.sqrt(
                    self.init_var
                )
        ...
        if self.read_mean_profile:
            ...
            assert os.path.isfile(self.mean_profile_meshbase + ".3dm"), (
                "couldn't find {f}".format(f=self.mean_profile_meshbase + ".3dm")
            )
            base3dm = os.path.basename(self.mean_profile_meshbase)
            dir3dm = os.path.dirname(self.mean_profile_meshbase)
            mesh_mean = Mesh_lite.Mesh2DM(
                os.path.join(dir3dm, base3dm.split(".")[0]), suffix="3dm"
            )
        ...
        for i in range(self.nrens):
            # mwf debug
            print(
                "Savannah River generate initial ensemble calling "
                "compute velocity for member {0}".format(i)
            )
    ...
    def compute_velocity_adh(self, zin, t):
        ...
        with open(self.sim_output, "w") as fout:
            fail = subprocess.call(
                [self.pre_adh_path, fileprefix], stdout=fout, stderr=fout
            )
            assert not fail
            fail = subprocess.call(
                [self.adh_path, fileprefix], stdout=fout, stderr=fout
            )
            assert not fail
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run assimimation on 2d regular domain using AdH simulations"
    )
    parser.add_argument(
        "--grid_gridgen",
    ...
    parser.add_argument(
        "--seed", help="random number seed", action="store", type=int, default=12345
    )
    parser.add_argument(
        "-R",
        "--rigid_lid",
...
""",
    ...
    forward_prob = SavannahRiver(
        grid_file=opts.grid_gridgen,
        rect_file=opts.rect_gridgen,
    ...
    if len(infilename.split(".")) > 1:
        outfilename = "{0}_uns.{1}".format(
            infilename.split(".")[0], infilename.split(".")[1]
        )
    else:
        outfilename = "{0}_uns".format(
            infilename.split(".")[0],
        )
    ...
    swp.extract_logically_rectangular_ensemble_to_unstructured(
        mesh, ensfile, ensfile_uns, opts.nrens, y_varies_fastest=ens_y_varies_fastest
    )
    ...
    t0 = 0.0
    # save measurement values projected to computational grid. Use mask
    # for non-measured values
    fortran_base = 1
    ...
    print(
        "Saving observations and indices to {0} and {1}".format(
            pdaf_obs_file, pdaf_obsind_file
        )
    )
    ...
    obs_nodeids = np.where(
        H.indices < prm.nn, H.indices, (H.indices - velocity_offset) / prm.velocity_dim
    )
    ...
    obs_state[velocity_offset :: prm.velocity_dim, :] = mesh.nodeArray[
        :, 0 : prm.velocity_dim
    ]
    obs_state[velocity_offset + 1 :: prm.velocity_dim, :] = mesh.nodeArray[
        :, 0 : prm.velocity_dim
    ]
