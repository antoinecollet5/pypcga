...
class Model:
    def __init__(self, params=None):
        ...
        if params is not None:
            ...
            if "adh_ntsim" in params:
                self.ntsim = params["adh_ntsim"]
            # inflow discharge and free surface elevation at the boundary
            # needed for writing initial conditions potentially
            if "z_f" in params:
                self.z_f = params["z_f"]
            if "Q_b" in params:
                self.Q_b = params["Q_b"]
            ...
            self.true_soln_file_h5 = (
                None
                if "true_soln_file_h5" not in params
                else params["true_soln_file_h5"]
            )
            self.true_soln_meshbase = (
                None
                if "true_soln_meshbase" not in params
                else params["true_soln_meshbase"]
            )
    ...
    def run_model(self, bathy, idx=0):
        ...
        forward_prob = setup_savannah.SavannahRiver(
            grid_file=self.adh_grid,
        ...
        ##instantiate the inverse problem which controls the forward model simulation
        prm = setup_savannah.SavannahRiverProblem(
            forward_prob.mesh,
            forward_prob,
            velocity_obs_loc,
            elev_obs_loc,
